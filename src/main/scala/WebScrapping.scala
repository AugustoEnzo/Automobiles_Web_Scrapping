  ...
  for (pg: Int <- Range(1, docMaxPage.toInt + 1, 1)) {
    ...
    for (num: Int <- Range(1, 55+1, 1)) {
      ...
      if (carIterator.text != "") {
        ...
        val carData: mutable.Buffer[String, mutable.Buffer[String], String, String, String, Option[Int], Option[Int], String, String,
          String, String, String, Option[Int], String, Option[Int], Option[Double], Option[Boolean], String, Option[Int],
          String, String, String, String] =
            ...
            innerPage.select(".bTSFxO").text.replace("cód. ", ""), // ID

            innerPage.select(".hSZkck").text.replace("Publicado em ", "").replace(" às ", ":")

        if (carData.head.toString.nonEmpty & carData.head != "https://static.olx.com.br/cd/listing/notFound.png"
          & carData(3).toString.nonEmpty) {

          println(s"${carData(22)}")
